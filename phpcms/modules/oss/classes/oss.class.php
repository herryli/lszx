...
class oss {
	public function upload(){
		//file_put_contents('a.txt', var_export($_FILES,true),FILE_APPEND);
		//print_r("<pre>");print_r($_FILES);print_r($_POST);exit;
		/**
		 * 加载sdk包以及错误代码包
		 */
		//require_once '../sdk.class.php';
		require_once PC_PATH.'modules'.DIRECTORY_SEPARATOR.'oss'.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.'alioss.class.php';
		...
		try{
			 ...
			 * Service相关操作
			 */
			//$this->get_service($oss_sdk_service);
	...
	function upload_by_file($obj){
                //file_put_contents('a.txt',var_export($_FILES,true),FILE_APPEND);
		$bucket = 'lszx';
		$object = "upload/".$_FILES['Filedata']['name'];	
		//$file_path = "E:\\download\\22.jpg";
		...
		$response = $obj->upload_file_by_file($bucket,$object,$file_path);
                ...
                //file_put_contents('a.txt',var_export($response,true),FILE_APPEND);
                //file_put_contents('a.txt','end------------------------------------'."\n",FILE_APPEND);

		$extend = pathinfo($_FILES['Filedata']['name']);
		
		$fileext =  strtolower($extend["extension"]);
		
		//$fileext = $response->header['x-oss-requestheaders']['Content-Type'];
		
		//file_put_contents('a.txt', var_export($_FILES,true),FILE_APPEND);
		
		
		if($fileext == 'rar' || $fileext == 'zip') $fileext = 'rar';
		...
		//file_put_contents('a.txt', $response->header['x-oss-request-id'].",".$response->header['x-oss-request-url'].",".$fileext.",".$_FILES['Filedata']['name'],FILE_APPEND);
	        	
		echo $response->header['x-oss-request-id'].",".$response->header['x-oss-request-url'].",".$fileext.",".$_FILES['Filedata']['name'];
		
		//图片传输
		//echo $response->header['x-oss-request-id'].",".$response->header['x-oss-request-url'].",1,".$_FILES['Filedata']['name'];
		
		//print_r("<pre>");print_r($response);print_r($response->header);print_r($response->body);exit;
		//_format($response);
	}
