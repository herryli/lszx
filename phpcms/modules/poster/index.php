...
class index {
	function __construct() {
		$this->space_db = pc_base::load_model('poster_space_model');
	...
	public function show_poster() {
		...
		//echo 'poster_template_'.$siteid;exit;
		$poster_template = getcache('poster_template_'.$siteid, 'commons');
		//exit;
		//print_r("<pre>");print_r($poster_template);exit;
		if ($poster_template[$r['type']]['option']) {
			...
			$space_data = $this->space_db->get_one(" `spaceid`='".$id."' AND `disabled`=0", '*');
			
			//print_r("<pre>");print_r(string2array($space_data['setting']));exit;
			
			if(is_array($space_data) && !empty($space_data)){
				$space_data['setting'] = string2array($space_data['setting']);
				$pinfo['space_setting']['paddleft'] = $space_data['setting']['paddleft'];
				$pinfo['space_setting']['paddtop'] = $space_data['setting']['paddtop'];
			}
			//print_r("<pre>");print_r($pinfo);exit;
