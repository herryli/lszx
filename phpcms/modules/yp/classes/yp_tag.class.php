		...
		if(!empty($pos_arr)) {
			foreach ($pos_arr as $info) {
				...
				if ($yp_setting['enable_rewrite']) {
					...
					if(in_array($info['modelid'],array(17,18))){
						$array[$key]['url'] = APP_PATH.'index.php?m=yp&c=index&a=show&catid='.$info['catid'].'&id='.$info['id'];
					}elseif($info['module'] == 'yp'){
						$temp_db = pc_base::load_model('yp_content_model');
						$temp_db->set_model($info['modelid']);
						$res = $temp_db->get_one(array('id'=>$info['id']));
						if(!empty($res)){
							$array[$key]['url'] = $res['url'];					
						}
					}elseif($info['module'] == 'content'){
						$array[$key]['url'] = APP_PATH.'index.php?m=content&c=index&a=show&catid='.$info['catid'].'&id='.$info['id'];
					}
				}
