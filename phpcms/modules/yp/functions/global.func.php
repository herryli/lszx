...
function get_companyinfo($userid, $fields = '*') {
	...
	if (!isset($usersinfo[$markid])) {
		...
		//print_r("<pre>");print_r($yp_company_db);exit;
		
		$usersinfo[$markid] = $yp_company_db->get_one(array('userid'=>$userid), $fields.',status');
		//print_r("<pre>");print_r($usersinfo);exit;
	}
 ...
 * 通过企业信息取得企业用户信息
 */
function get_company_list($limit = 2) {
		$yp_company_db = pc_base::load_model('yp_company_model');
		$res = $yp_company_db->select($where = '', $data = '*', $limit, $order = 'id DESC', $group = '', $key='');
		return $res;
}
...
function compute_company_url($action = '', $array = array()) {
	...
	$member_res = $member->get_one(array('userid'=>$userid), 'username');
	$username = $member_res['username'];
	//file_put_contents("a.txt",$member_res['username'],FILE_APPEND);exit;
	//print_r("<pre>");print_r($member_res);exit;
	if (!$com_url[$userid]) {
		...
		$user_url = $r['url'];
		//file_put_contents("a.txt", $user_url."<br>".$username.".".URL_PATH);
		if (strpos($user_url, $username.".".URL_PATH)===false && $user_url) {
...
//更新企业主页导航
function company_menu ($companyinfo,$username) {
	...
	if (strpos($companyurl, $username.".".URL_PATH)===false && $companyurl) {
		...
		$pre_url = "http://".$username.".".URL_PATH;
	}
		...
		if ($p['is_system']) {
			...
			if (in_array($p['id'], $user_arr)) {
				...
				if ($pre_url) {
					...
					if ($setting['enable_rewrite']) {
						$company_meun['catname'][$k]['linkurl'] = $pre_url.'web-'.$update_user[$p['catname']].'----'.$userid.'-1.html';
					} else {
				...
				foreach ($yp_models as $mid => $m) {
					if ($mid==$p['id']) {
						if ($pre_url) {
							if ($setting['enable_rewrite']) {
								$company_meun['catname'][$k]['linkurl'] = $pre_url.'web-model-'.$mid.'---'.$userid.'-1.html';
							} else {
