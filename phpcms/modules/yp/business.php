...
class business extends common {
	...
	function __construct() {
		parent::__construct();
		//print_r("<pre>");print_r($this->_username);exit;
	...
	public function register() {
		//echo $this->_username.".".URL_PATH.'web-'.$userid.'.html';exit;
		if (isset($_POST['dosubmit'])) {
			...
			//print_r("<pre>");print_r($yp_setting);exit;
			...
			//计算公司导航菜单及默认模板
			$yp_models = getcache('yp_model', 'model');
			//print_r("<pre>");print_r($yp_models);exit;
			$menu_user = array();
			...
			if ($this->setting['enable_rewrite']) {
				$menu_user['list'][$n] = $n;
				$menu_user['catname'][$n] = array('used'=>1, 'id'=>'index', 'is_system'=>1, 'catname'=>L('first'), 'linkurl'=>"http://".$username.".".URL_PATH.'web-'.$userid.'.html');
				$n++;
			...
			//print_r("<pre>");print_r($menu_user);exit;
			//获取默认模板
			...
			if ($yp_setting['enable_rewrite']) {
				$url = "http://".$username.".".URL_PATH.'web-'.$userid.'.html';
				//$url = APP_PATH.'web-'.$userid.'.html';
			} else {
				$url = "http://".$username.".".URL_PATH.'index.php?m=yp&c=com_index&userid='.$userid;
	...
	public function menu(){
		...
		if($_POST['submit']){
			...
			$menu = company_menu($array,$username);
	...
	public function content() {
		...
		switch ($action) {
			case 'list':
				...
				//$temp_var =  var_export($datas,true);
				//file_put_contents('a.txt',$temp_var,FILE_APPEND);
			...
			case 'add':
				...
				if (isset($_POST['dosubmit'])) {
					...
					$forminfos_data = $yp_form->get();
					//print_r("<pre>");print_r($forminfos_data);exit;
					$forminfos = array();
					foreach($forminfos_data as $_fk=>$_fv) {
						if($_fv['isomnipotent']) continue;
						if($_fv['formtype']=='omnipotent') {
