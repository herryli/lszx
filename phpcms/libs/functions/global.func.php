...
function L($language = 'no_language',$pars = array(), $modules = '') {
	...
	if(!$LANG) {
		//echo PC_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.'system.lang.php'."<br>";
		//echo PC_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.ROUTE_M.'.lang.php';
...
function debug_print($data){
	$temp_var = var_export($data,true);	
	file_put_contents('a.txt',$temp_var,FILE_APPEND);
}
/**
 * 通过用户名获取公司信息
 * @param string $username
 * @param int $only_name 1代表只需要公司名称 0代表需要全部信息
 */
function get_company_by_username($username ='',$only_name = 1){
	if($username != ''){
		//获取USERID
		$member_db = pc_base::load_model('member_model');
		...
		$infos = $yp_company->listinfo($where,$order = 'regtime desc');
		if($only_name == 1){
			if(isset($infos[0])){
