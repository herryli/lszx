...
class param {
	 ...
	 * 
	 */
	public function realm_name() {
		$host = $_SERVER['HTTP_HOST'];
		preg_match("/^[^\.]+/", $host, $matches);
		$username = $matches[0];
		$member = pc_base::load_model('member_model_route');
		$member_res = $member->get_one(array('username'=>$username), 'userid');
		
		if(!preg_match("/^\/.+/",$_SERVER['REQUEST_URI'],$mat) && !empty($member_res)){
			$userid = $member_res['userid'];
			$_GET['m'] = "yp";
			$_GET['c'] = "com_index";
			$_GET['a'] = "init";
			$_GET['userid'] = $userid;
